package test.biz.praybill.client;
...
public class PrayBillUI extends PFToftPanel {
	...
	public PrayBillUI(Container parent) {
	...
	public void onButtonClicked(ButtonObject bo) {
		if (bo.getCode().equalsIgnoreCase("New")){
			...
			try {
				collectData();
				po = NCLocator.getInstance().lookup(IServicesInterface.class).SavePrayBill(po);
				initializeData(po);
				tablePane.updateUI();
				...
				try {
					...
					e.printStackTrace();
					MessageBox.showMessageDialog("error", e.getMessage());
				}
			...
			FlowStateDlg dlg;
			try {
				dlg = new FlowStateDlg(this, null, po.getParentVO().getPrimaryKey(),4);
				dlg.showModal();
			} catch (BusinessException e) {
				e.printStackTrace();
				MessageBox.showMessageDialog("error", e.getMessage());
			...
			if (billID != null){
				try {
					po = NCLocator.getInstance().lookup(IServicesInterface.class).getPrayBill(billID);
				} catch (BusinessException e) {
					e.printStackTrace();
					MessageBox.showMessageDialog("error", e.getMessage());
				}
	...
	private void initializeUI(){
		...
		pnlHeader.setPreferredSize(new Dimension(200,30));
		//pnlHeader.setLayout(new BorderLayout());
		UILabel labelID = new UILabel("Pray Bill ID");
	...
	private void initializeData(AggTestPrayBill data){
		this.textID.setValue(((TestPrayBillMaster)data.getParentVO()).getId());
		this.textSupplier.setValue(((TestPrayBillMaster)data.getParentVO()).getSupplier());
		this.textSum.setValue(((TestPrayBillMaster)data.getParentVO()).getSum().toString());
		tableModel.setRow(Arrays.asList(po.getChildrenVO()));
	}

	private void collectData(){
		//this.textID.setValue(((TestPrayBillMaster)data.getParentVO()).getId());
		((TestPrayBillMaster)po.getParentVO()).setSupplier(this.textSupplier.getValue().toString());
		((TestPrayBillMaster)po.getParentVO()).setSum(new UFDouble(this.textSum.getValue().toString()));
		//tableModel.setRow(Arrays.asList(po.getChildrenVO()));
	}
	...
	class PrayBillTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			TestPrayBillDetail p = (TestPrayBillDetail)propertiesList.get(row);
