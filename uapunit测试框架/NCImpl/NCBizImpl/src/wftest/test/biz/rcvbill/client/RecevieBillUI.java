package test.biz.rcvbill.client;
...
public class RecevieBillUI extends PFToftPanel {
	...
	private AggTestRcvBill po;
	
	private ButtonObject[] buttons;
	
	private PrayBillTableModel tableModel = null;
	
	public RecevieBillUI() {
		this(null);
	}
	public RecevieBillUI(Container parent) {
	...
	private AggTestRcvBill queryData(String id){
		...
		if (id != null && !id.isEmpty()){
			...
			TestRcvBillMaster master = new TestRcvBillMaster();
			master.setSupplier("newSupplier");
			master.setSum(new UFDouble(0.0));
			tempAggVO.setParentVO(master);
			
			TestRcvBillDetail detail = null;
			int count = 3;
			TestRcvBillDetail[] details = new TestRcvBillDetail[count];
			for(int n = 0; n < count; n++){
				detail = new TestRcvBillDetail();
	...
	private void collectData(){
		...
		((TestRcvBillMaster)po.getParentVO()).setSupplier(this.textSupplier.getValue().toString());
		((TestRcvBillMaster)po.getParentVO()).setSum(new UFDouble(this.textSum.getValue().toString()));
	...
	private void initializeUI(){
		...
		UILabel labelID = new UILabel("Recevie Bill ID");
	...
	private void initializeData(AggTestRcvBill data){
		this.textID.setValue(((TestRcvBillMaster)data.getParentVO()).getId());
		this.textSupplier.setValue(((TestRcvBillMaster)data.getParentVO()).getSupplier());
		this.textSum.setValue(((TestRcvBillMaster)data.getParentVO()).getSum().toString());
		tableModel.setRow(Arrays.asList(po.getChildrenVO()));
	}
	...
	class PrayBillTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			TestRcvBillDetail p = (TestRcvBillDetail)propertiesList.get(row);
