 ...
 */
EditorUi = function(editor, container,style)
{
	window.statue="";
...
/**
 * Specifies the width of the propertyGrid. Default is 190.
 */
EditorUi.prototype.propertyGridWidth = 250;
 ...
 */
EditorUi.prototype.saveAndDeploy = function()
{
	var enc = new mxCodec(mxUtils.createXmlDocument());
	enc.encodeDefaults = true;
	enc.lowercaseElementName = true;
	var modelManager = new ModelManager(enc, this.editor.graph.getModel().getRoot());
	var root = modelManager.toXmlDocument();
	var xml = mxUtils.getXml(root);
	try
	{
		if (xml.length < MAX_REQUEST_SIZE){
			xml = encodeURIComponent(xml);
			mxUtils.post(SAVEDEPLOY_URL, 'id=' + modelManager.getMainProcess().getId() + '&xml=' + xml, mxUtils.bind(this, function(req)
			{
				if(req.getStatus() == 404)
				alert(req);
			}));
			//new mxXmlRequest(SAVEDEPLOY_URL, 'xml=' + xml).simulate(document, "_blank");
			//new mxXmlRequest(SAVE_URL, 'filename=xry.xml&xml=' + xml).simulate(document, "_blank");
		}
		else{
...
EditorUi.prototype.refresh = function()
{
	...
	if (quirks)
	{
		...
		this.diagramContainer.style.width = (w - effHsplitPosition - this.splitSize - this.propertyGridWidth - this.splitSize) + 'px';
	...
	{
		if(mxClient.IS_MT||mxClient.IS_SAF)
		{
		  this.diagramContainer.style.width = (w - effHsplitPosition - this.splitSize - this.propertyGridWidth - this.splitSize) + 'px';
		  var diagramHeight = (h - this.footerHeight - this.menubarHeight - this.toolbarHeight);
		  this.diagramContainer.style.height = diagramHeight+ 'px';
	...
	this.hsplitRight.style.top = this.sidebarContainer.style.top;
	this.hsplitRight.style.bottom = this.outlineContainer.style.bottom;
	this.hsplitRight.style.left = (w - this.propertyGridWidth - this.splitSize) + 'px';
	this.hsplitRight.style.height = diagramHeight + 'px';
	$('#'+this.propertyContainer.id).propertygrid('resize',{width:this.propertyGridWidth});
	this.propertyContainer.style.left = (w - this.propertyGridWidth) + 'px';
	this.propertyContainer.style.top = this.sidebarContainer.style.top;
	this.propertyContainer.style.width = this.propertyGridWidth+ 'px';
	this.propertyContainer.style.height = diagramHeight+ 'px';
	if(this.propertyContainer.parentElement != null)
	{
		$('.propertygrid')[0].style.width = this.propertyGridWidth+ 'px';
		$('.propertygrid')[0].style.left = (w - this.propertyGridWidth) + 'px';
	}
	
	var grid = $.data(this.propertyContainer,"propertygrid");
	if(grid && grid.options)
	{
		grid.options.left = (w - this.propertyGridWidth);
		grid.options.top = this.sidebarContainer.style.top;
		grid.options.width = this.propertyGridWidth;
		grid.options.height = diagramHeight;
		var colOptionValue = $('#'+this.propertyContainer.id).propertygrid('getColumnOption', 'value');
		var colOptionName = $('#'+this.propertyContainer.id).propertygrid('getColumnOption', 'name');
		var colOptionFix = $('#'+this.propertyContainer.id).propertygrid('getColumnOption', 'f');
		var colWidthValue =  this.propertyGridWidth - colOptionFix.width - colOptionName.width - 18/*scroll*/;
		colOptionName.width = Math.min(colWidthValue,100);
		if (colWidthValue > 20/*min*/){
			colWidthValue.resizable = true;
			
			colOptionName.resizable = true;
			colOptionValue.boxWidth = colWidthValue - 2;
			colOptionValue.width = colWidthValue;
		}
	}
};
...
/*
	...
	this.propertyContainer.id = 'grid-container';
	* */
	this.propertyContainer = document.createElement('table');
	this.propertyContainer.className = 'easyui-propertygrid';
	this.propertyContainer.id = 'grid-container';

	this.footerContainer = this.createDiv('geFooterContainer');
	this.hsplit = this.createDiv('geHsplit');
	
	this.hsplitRight = this.createDiv('geHsplit');
...
EditorUi.prototype.createUi = function()
{
	...
	this.container.appendChild(this.hsplitRight);
	this.container.appendChild(this.vsplit);
	/*
    this.propertyGrid = Ext.create('Ext.grid.property.Grid', {
    width: this.propertyGridWidth,
    renderTo: this.propertyContainer.id,
    propertyNames: {
      tested: 'QA',
	...
	*/
	$('#'+this.propertyContainer.id).propertygrid({
		url:"",
		showGroup:true, 
		scrollbarSize:0,
		columns:[[
		{field:'displayname',title:mxResources.get('propertyDisplayName'),width:100,sortable:true},
		{field:'value',title:mxResources.get('propertyValue'),width:100,resizable:true},
		{field:'name',title:mxResources.get('propertyName'),width:0,sortable:true,hidden:true}
		]]
	});
	
	//$('#'+this.propertyContainer.id).propertygrid('fitColumns');
	
    this.editor.graph.setPropertyGrid(this.propertyContainer.id);
	...
	// HSplitRight
	this.addSplitHandler(this.hsplitRight, true, 0, mxUtils.bind(this, function(value)
	{
		this.propertyGridWidth = this.container.clientWidth - value - this.splitSize;
		//alert(this.propertyGridWidth);
		this.refresh();
		this.editor.graph.sizeDidChange();
		this.editor.outline.update(false);
		this.editor.outline.outline.sizeDidChange();
