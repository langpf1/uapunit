...
public class JPAEntityMappings {
  ...
  public String getJPAClassString(Object value) {
    if(value == null) {
      throw new WorkflowException("null value cannot be saved");
    }
    
    EntityMetaData metaData = getEntityMetaData(value.getClass());
    if(!metaData.isJPAEntity()) {
      throw new WorkflowException("Object is not a JPA Entity: class='" + value.getClass() + "', " + value);
    }
  ...
  private Object getIdValue(Object value, EntityMetaData metaData) {
    try {
      ...
      throw new WorkflowException("Illegal argument exception when getting value from id method/field on JPAEntity", iae);
    } catch (IllegalAccessException iae) {
      throw new WorkflowException("Cannot access id method/field for JPA Entity", iae);
    } catch (InvocationTargetException ite) {
      throw new WorkflowException("Exception occured while getting value from id field/method on JPAEntity: " + 
        ite.getCause().getMessage(), ite.getCause());
    }
    
    // Fall trough when no method and field is set
    throw new WorkflowException("Cannot get id from JPA Entity, no id method/field set");
  }
  ...
  public Object getJPAEntity(String className, String idString) {
    ...
    if(metaData == null) {
      throw new WorkflowException("Class is not a JPA-entity: " + className);
    }
  ...
  private Object findEntity(Class< ? > entityClass, Object primaryKey) {
    ...
    if(entity == null) {
      throw new WorkflowException("Entity does not exist: " + entityClass.getName() + " - " + primaryKey);
    }
    return entity;
  }
  ...
  public Object createId(EntityMetaData metaData, String string) {
    ...
    if(type == Long.class || type == long.class) {
      ...
      throw new WorkflowException("Unsupported Primary key type for JPA-Entity: " + type.getName());
    }
  }
  
  public String getIdString(Object value) {
    if(value == null) {
      throw new WorkflowException("Value of primary key for JPA-Entity cannot be null");
    }
    ...
    if(value instanceof java.util.Date){
      ...
      throw new WorkflowException("Unsupported Primary key type for JPA-Entity: " + value.getClass().getName());
    }
  }
}
