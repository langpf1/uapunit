  ...
  public void flush() {
    if (entityManager != null && (!handleTransactions || isTransactionActive()) ) {
      try {
        ...
        throw new WorkflowException("Error while flushing EntityManager, illegal state", ise);
      } catch (TransactionRequiredException tre) {
        throw new WorkflowException("Cannot flush EntityManager, an active transaction is required", tre);
      } catch (PersistenceException pe) {
        throw new WorkflowException("Error while flushing EntityManager: " + pe.getMessage(), pe);
      }
    }
  }
  ...
  public void close() {
    if (closeEntityManager && entityManager != null && !entityManager.isOpen()) {
      try {
        ...
        throw new WorkflowException("Error while closing EntityManager, may have already been closed or it is container-managed", ise);
      }
    }
  }
