...
public interface SuspensionState {
  ...
  public static class SuspensionStateUtil{
    public static void setSuspensionState(IProcessDefinition processDefinitionEntity, SuspensionState state) {
      if(processDefinitionEntity.getSuspensionState() == state.getStateCode()) {
        throw new WorkflowException("Cannot set suspension state '"+state+"' for "+processDefinitionEntity+"': already in state '"+state+"'.");
      }
      processDefinitionEntity.setSuspensionState(state.getStateCode());
    }   
    public static void setSuspensionState(ActivityInstanceEntity executionEntity, SuspensionState state) {
			// if(executionEntity.getSuspensionState() == state.getStateCode())
			// {
			// throw new
			// ActivitiException("Cannot set suspension state '"+state+"' for "+executionEntity+"': already in state '"+state+"'.");
			// }
