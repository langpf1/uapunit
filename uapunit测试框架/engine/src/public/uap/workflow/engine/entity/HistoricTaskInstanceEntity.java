...
public class HistoricTaskInstanceEntity extends HistoricScopeInstanceEntity implements HistoricTaskInstance, PersistentObject {
	...
	public HistoricTaskInstanceEntity(ITask task, IActivityInstance execution) {
		this.id = task.getTaskPk();
		if (execution != null) {
			this.processDefinitionId = execution.getProcessDefinition().getProDefId();
			this.processInstanceId = execution.getProcessInstance().getProInsPk();
			this.executionId = execution.getActInsPk();
		}
		this.name = task.getName();
		this.parentTaskId = task.getParentTask().getTaskPk();
		this.description = task.getDescription();
		this.owner = task.getOwner();
		// this.assignee = task.getAssignee();
		this.startTime = ClockUtil.getCurrentTime();
		this.taskDefinitionKey = task.getTaskDefinition().getKey();
