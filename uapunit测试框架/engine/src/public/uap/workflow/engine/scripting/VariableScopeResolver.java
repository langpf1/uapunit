...
public class VariableScopeResolver implements Resolver {
	...
	public VariableScopeResolver(VariableScope variableScope) {
		if (variableScope == null) {
			throw new WorkflowException("variableScope cannot be null");
		}
		if (variableScope instanceof IActivityInstance) {
			variableScopeKey = "execution";
		} else if (variableScope instanceof ITask) {
			variableScopeKey = "task";
		} else {
			throw new WorkflowException("unsupported variable scope type: " + variableScope.getClass().getName());
		}
