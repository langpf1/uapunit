...
public class ScriptCondition implements Condition {
	...
	public boolean evaluate(IActivityInstance execution) {
		...
		if (result == null) {
			throw new WorkflowException("condition script returns null: " + expression);
		}
		if (!(result instanceof Boolean)) {
			throw new WorkflowException("condition script returns non-Boolean: " + result + " (" + result.getClass().getName() + ")");
		}
