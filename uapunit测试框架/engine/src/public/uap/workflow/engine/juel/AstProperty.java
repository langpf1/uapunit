...
public abstract class AstProperty extends AstNode {
	...
	public void appendStructure(StringBuilder b, Bindings bindings,ELContext context) {
		getChild(0).appendStructure(b, bindings);
		b.append(".");
		b.append(((AstProperty) this).getProperty(bindings, context));
	}

	@Override
	public Object eval(Bindings bindings, ELContext context) {
		StringBuilder combinedName = new StringBuilder();
		appendStructure(combinedName,  bindings, context);
		Object property = combinedName.toString();
		Object result = context.getELResolver().getValue(context, null, property);
		/*
		/*AstNode currentNode = this;
		while (!(currentNode instanceof AstIdentifier)) {
			if (currentNode instanceof AstProperty) {
				combinedName = ((AstProperty) currentNode).getProperty(bindings, context) + "." + combinedName;
				currentNode = ((AstProperty) currentNode).getPrefix();
			}
		}
		if ((currentNode instanceof AstIdentifier)) {
			combinedName = currentNode.toString() + "." + combinedName;
		}
