...
public class FormPropertyHandler {
	...
	public FormProperty createFormProperty(IActivityInstance execution) {
	...
	public void submitFormProperty(IActivityInstance execution, Map<String, String> properties) {
		if (!isWritable && properties.containsKey(id)) {
			throw new WorkflowException("form property '" + id + "' is not writable");
		}
		if (isRequired && !properties.containsKey(id) && defaultExpression == null) {
			throw new WorkflowException("form property '" + id + "' is required");
		}
		Object modelValue = null;
