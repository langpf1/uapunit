...
public class ParallelGatewayActivityBehavior extends GatewayActivityBehavior {
	...
	public void execute(IActivityInstance execution) throws Exception {
		...
		List<IActivityInstance> completedJoinedExecutions = execution.findIncomeCompletedExecutions(
				execution.getProcessInstance(), activity);
		List<IActivityInstance> joinedExecutions = execution.findInactiveConcurrentExecutions(activity);
		int nbrOfExecutionsToJoin = execution.getActivity().getIncomingTransitions().size();
		int nbrOfExecutionsJoined = completedJoinedExecutions.size();
