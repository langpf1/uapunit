...
public class ScopeUtil {
	 ...
	 */
	public static IActivityInstance findScopeExecution(IActivityInstance execution, IScope scopeActivity) {
		// TODO: this feels hacky!
		if (scopeActivity instanceof IProcessDefinition) {
			return null;
		} else {
			IActivity currentActivity = execution.getActivity();
			IActivityInstance candiadateExecution = null;
			IActivityInstance originalExecution = execution;
	...
	public static IActivity findInParentScopesByBehaviorType(ActivityImpl activity, Class<? extends ActivityBehavior> behaviorType) {
		while (activity != null) {
			for (IActivity childActivity : activity.getActivities()) {
				if (behaviorType.isAssignableFrom(childActivity.getActivityBehavior().getClass())) {
					return childActivity;
				}
			}
			activity = (ActivityImpl) activity.getParentActivity();
		}
		return null;
	}
	public static void destroyScope(IActivityInstance execution, String reason) {
		List<IActivityInstance> executions = execution.getExecutions();
		for (IActivityInstance childExecution : executions) {
	 ...
	 */
	public static void throwCompensationEvent(List<CompensateEventSubscriptionEntity> eventSubscriptions, IActivityInstance execution, boolean async) {
		// first spawn the compensating executions
		for (EventSubscriptionEntity eventSubscription : eventSubscriptions) {
			ActivityInstanceEntity compensatingExecution = null;
			...
			if (eventSubscription.getConfiguration() != null) {
				compensatingExecution = Context.getCommandContext().getExecutionManager().getActInsByActInsPk(eventSubscription.getConfiguration());
				// move the compensating execution under this execution:
				compensatingExecution.setParent((ActivityInstanceEntity) execution);
				compensatingExecution.setEventScope(false);
			} else {
				compensatingExecution = (ActivityInstanceEntity) execution.createExecution(execution.getActivity());
				compensatingExecution.setPk_actins(execution.getActInsPk());
				eventSubscription.setConfiguration(compensatingExecution.getActInsPk());
				compensatingExecution = Context.getCommandContext().getExecutionManager().getActInsByActInsPk(eventSubscription.getConfiguration());
				// move the compensating execution under this execution:
				compensatingExecution.setParent((ActivityInstanceEntity) execution);
				compensatingExecution.setEventScope(false);
	 ...
	 * scope
	 */
	public static void createEventScopeExecution(ActivityInstanceEntity execution) {
		IActivityInstance eventScope = ScopeUtil.findScopeExecution(execution, execution.getActivity().getParent());
		if (eventScope == null) {
			return;
		}
		List<CompensateEventSubscriptionEntity> eventSubscriptions = execution.getCompensateEventSubscriptions();
		if (eventSubscriptions.size() > 0) {
			IActivityInstance eventScopeExecution = eventScope.createExecution(eventScope.getActivity());
			...
			eventSubscription.setConfiguration(eventScopeExecution.getActInsPk());
		}
	}
}
