...
public class MailActivityBehavior extends AbstractBpmnActivityBehavior {
  ...
  protected Email createEmail(String text, String html) {
    if (html != null) {
      ...
      throw new WorkflowException("'html' or 'text' is required to be defined when using the mail activity");
    }
  }
  ...
  protected SimpleEmail createTextOnlyEmail(String text) {
    ...
    try {
      ...
      throw new WorkflowException("Could not create text-only email", e);
    }
  }
  ...
  protected void setFrom(Email email, String from) {
    ...
    try {
      ...
      throw new WorkflowException("Could not set " + from + " as from address in email", e);
    }
  }
    ...
    if (ccs != null) {
      for (String c : ccs) {
        try {
          ...
          throw new WorkflowException("Could not add " + c + " as cc recipient", e);
        }
      }
    }
  }
  ...
  protected String getStringFromField(Expression expression, IActivityInstance execution) {
    if(expression != null) {
