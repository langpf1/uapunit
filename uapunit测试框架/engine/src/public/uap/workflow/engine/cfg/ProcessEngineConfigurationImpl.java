...
import uap.workflow.engine.bpmn.deployer.BpmnDeployer;
import uap.workflow.engine.bpmn.parser.BpmnParseListener;
import uap.workflow.engine.bpmn.parser.BpmnParser;
import uap.workflow.engine.bpmn.webservice.MessageInstance;
import uap.workflow.engine.calendar.BusinessCalendarManager;
import uap.workflow.engine.calendar.CycleBusinessCalendar;
import uap.workflow.engine.calendar.DueDateBusinessCalendar;
import uap.workflow.engine.calendar.DurationBusinessCalendar;
import uap.workflow.engine.calendar.MapBusinessCalendarManager;
import uap.workflow.engine.db.DbSqlSessionFactory;
import uap.workflow.engine.db.IbatisVariableTypeHandler;
import uap.workflow.engine.db.UapPkGenerator;
...
import uap.workflow.engine.event.SignalEventHandler;
import uap.workflow.engine.exception.WorkflowException;
import uap.workflow.engine.form.AbstractFormType;
import uap.workflow.engine.form.BooleanFormType;
import uap.workflow.engine.form.DateFormType;
import uap.workflow.engine.form.FormEngine;
import uap.workflow.engine.form.FormTypes;
import uap.workflow.engine.form.JuelFormEngine;
import uap.workflow.engine.form.LongFormType;
import uap.workflow.engine.form.StringFormType;
import uap.workflow.engine.handler.HistoryParseListener;
import uap.workflow.engine.interceptor.CommandContextFactory;
import uap.workflow.engine.interceptor.CommandExecutor;
import uap.workflow.engine.interceptor.CommandExecutorImpl;
import uap.workflow.engine.interceptor.CommandInterceptor;
import uap.workflow.engine.interceptor.DelegateInterceptor;
import uap.workflow.engine.interceptor.SessionFactory;
import uap.workflow.engine.invocation.DefaultDelegateInterceptor;
import uap.workflow.engine.io.IoUtil;
import uap.workflow.engine.jobexecutor.AsyncContinuationJobHandler;
import uap.workflow.engine.jobexecutor.DefaultJobExecutor;
...
import uap.workflow.engine.mail.MailScanner;
...
import uap.workflow.engine.persistence.GenericManagerFactory;
import uap.workflow.engine.query.FormServiceImpl;
import uap.workflow.engine.query.HistoryServiceImpl;
import uap.workflow.engine.query.IdentityServiceImpl;
import uap.workflow.engine.query.ManagementServiceImpl;
import uap.workflow.engine.query.ProcessEngineImpl;
import uap.workflow.engine.query.RepositoryServiceImpl;
import uap.workflow.engine.query.RuntimeServiceImpl;
import uap.workflow.engine.query.ServiceImpl;
import uap.workflow.engine.query.TaskServiceImpl;
import uap.workflow.engine.scripting.BeansResolverFactory;
import uap.workflow.engine.scripting.ResolverFactory;
import uap.workflow.engine.scripting.ScriptBindingsFactory;
import uap.workflow.engine.scripting.ScriptingEngines;
import uap.workflow.engine.scripting.VariableScopeResolverFactory;
...
import uap.workflow.engine.variable.BooleanType;
import uap.workflow.engine.variable.ByteArrayType;
import uap.workflow.engine.variable.CustomObjectType;
import uap.workflow.engine.variable.DateType;
import uap.workflow.engine.variable.DefaultVariableTypes;
import uap.workflow.engine.variable.DoubleType;
import uap.workflow.engine.variable.EntityManagerSession;
import uap.workflow.engine.variable.EntityManagerSessionFactory;
import uap.workflow.engine.variable.IntegerType;
import uap.workflow.engine.variable.JPAEntityVariableType;
import uap.workflow.engine.variable.LongType;
import uap.workflow.engine.variable.NullType;
import uap.workflow.engine.variable.SerializableType;
import uap.workflow.engine.variable.ShortType;
import uap.workflow.engine.variable.StringType;
...
public abstract class ProcessEngineConfigurationImpl extends ProcessEngineConfiguration {
	...
	protected ExtensionConfig extensionconfig = null;
	// buildProcessEngine
	// ///////////////////////////////////////////////////////
	...
	protected CommandInterceptor initInterceptorChain(List<CommandInterceptor> chain) {
		if (chain == null || chain.isEmpty()) {
			throw new WorkflowException("invalid command interceptor chain configuration: " + chain);
		}
		...
		if (dataSource == null) {
			if (dataSourceJndiName != null) {
				try {
					...
					throw new WorkflowException("couldn't lookup datasource from " + dataSourceJndiName + ": " + e.getMessage(), e);
				}
	...
	public void initDatabaseType() {
		...
		try {
			...
			if (databaseType == null) {
				throw new WorkflowException("couldn't deduct database type from database product name '" + databaseProductName + "'");
			}
	...
	public void initExtensionConfig(){
		String configPath = "uap/workflow/engine/cfg/BizImpl.cfg.xml";
		Reader reader = null;
		try {
			...
			JAXBContext jc;
			try {
				jc = JAXBContext.newInstance(ExtensionConfig.class);
				...
				// 开始解析
				extensionconfig = (ExtensionConfig) us.unmarshal(streamReader);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void initHistoryLevel() {
		if (HISTORY_NONE.equalsIgnoreCase(history)) {
			...
			throw new WorkflowException("invalid history level: " + history);
		}
	}
	// id generator
	// /////////////////////////////////////////////////////////////
	protected void initIdGenerator() {
		if (idGenerator == null) {
			UapPkGenerator dbIdGenerator = new UapPkGenerator();
	...
	protected void initExpressionManager() {
		if (expressionManager == null) {
			expressionManager = new ExtExpressionManager();//流程引擎初始化的时候，执行
		}
	}
