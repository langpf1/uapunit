...
import uap.workflow.engine.util.LogUtil.ThreadLogMode;
/**
 * @author Tom Baeyens
 */
public abstract class AbstractWorkflowTestCase extends PvmTestCase {
	...
	public void assertProcessEnded(final String processInstanceId) {
		IProcessInstance processInstance = processEngine.getRuntimeService().createProcessInstanceQuery().processInstanceId(processInstanceId).singleResult();
	...
	public void waitForJobExecutorToProcessAllJobs(long maxMillisToWait, long intervalMillis) {
		...
		try {
			...
			if (areJobsAvailable) {
				throw new WorkflowException("time limit of " + maxMillisToWait + " was exceeded");
			}
		} finally {
	...
	public void waitForJobExecutorOnCondition(long maxMillisToWait, long intervalMillis, Callable<Boolean> condition) {
		...
		try {
			...
			try {
				...
				throw new WorkflowException("Exception while waiting on condition: " + e.getMessage(), e);
			} finally {
				timer.cancel();
			}
