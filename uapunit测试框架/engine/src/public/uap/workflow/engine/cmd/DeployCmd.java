...
public class DeployCmd<T> implements Command<Deployment> {
	...
	public Deployment execute(CommandContext commandContext) {
		...
		if (deploymentBuilder.isDuplicateFilterEnabled()) {
			DeploymentEntity existingDeployment = Context.getCommandContext().getDeploymentManager().findLatestDeploymentByName(deployment.getName());
			if ((existingDeployment != null) && !deploymentsDiffer(deployment, existingDeployment)) {
				return existingDeployment;
			}
		}
		deployment.setNew(true);
		deployment.setDraft(deploymentBuilder.isDraft());
