...
import uap.workflow.modeler.uecomponent.IPushOkandCancelListener;
...
public class ModelerPropertySheetTable extends UITable implements IPushOkandCancelListener{
	...
	public ModelerPropertySheetTable() {
		this(new PropertySheetTableModel());
	}

	public ModelerPropertySheetTable(PropertySheetTableModel dm) {
		super(dm);
		addMouseListener(new CategoryVisibilityToggle());

		setGridColor(BpmnModelerConstants.TABLEGRID_COLOR);
	...
	public int getRowHeight() {
		return BpmnModelerConstants.ROW_HIGHT;
	}
	...
	public final TableCellEditor getCellEditor(int row, int column) {
		...
		if (o instanceof Property) {
			...
			if (editor != null) {
				if (editor instanceof AbstractDailogPropertyEditor) {
//					((AbstractDailogPropertyEditor) editor).getOKCancelDlg().addPushOkandCancelListener(ModelerPropertySheetTable.this);
				}
	...
	public final TableCellRenderer getCellRenderer(int row, int column) {
		...
		if (o instanceof PropertySheetTableModel.Category) {
			...
			PropertyEditor editor = getEditorRegistry().getEditor(property);
			if (editor != null && editor instanceof ICustomRendererEdit) {
				renderer = ((ICustomRendererEdit) editor).getRender();
			}else if (editor instanceof AbstractDailogPropertyEditor){
				renderer = ((AbstractDailogPropertyEditor)editor).getRender();
			}
			if (renderer == null) {
	...
	private transient TableModelListener cancelEditing = new TableModelListener() {
		...
		public void tableChanged(TableModelEvent e) {
			...
			if (e.getType() == TableModelEvent.UPDATE) {
				...
				int editingRow = ModelerPropertySheetTable.this.getEditingRow();
				TableCellEditor editor = ModelerPropertySheetTable.this.getCellEditor();
	...
	private static class CategoryVisibilityToggle extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent event) {
			...
			ModelerPropertySheetTable table = (ModelerPropertySheetTable) event.getComponent();
			int row = table.rowAtPoint(event.getPoint());
			int column = table.columnAtPoint(event.getPoint());
			if (row != -1 && column == 0 && event.getX() < BpmnModelerConstants.HOTSPOT_SIZE) {
	...
	private static class CellBorder implements Border {
		...
		public Insets getBorderInsets(Component c) {
			return new Insets(1,  BpmnModelerConstants.HOTSPOT_SIZE, 1, 1);
		}

		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
			Color oldColor = g.getColor();
			g.setColor(background);
			g.fillRect(x, y, x +  BpmnModelerConstants.HOTSPOT_SIZE - 2, y + height);
			g.setColor(oldColor);

			if (showToggle) {
				if (toggleState) {
					expandedIcon.paintIcon(c, g, x + ( BpmnModelerConstants.HOTSPOT_SIZE - 2 - expandedIcon.getIconWidth()) / 2, y + (height - expandedIcon.getIconHeight()) / 2);
				} else {
					collapsedIcon.paintIcon(c, g, x + ( BpmnModelerConstants.HOTSPOT_SIZE - 2 - collapsedIcon.getIconWidth()) / 2, y + (height - collapsedIcon.getIconHeight()) / 2);
				}
			}
		}
	...
	private static class PropertyRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			setBorder(new CellBorder(UIManager.getColor(BpmnModelerConstants.BACKGROUND_COLOR_KEY)));
			return this;
		}
	}
	...
	private static class CategoryRenderer extends DefaultTableCellRenderer {
		...
		public CategoryRenderer() {
			this(UIManager.getColor(BpmnModelerConstants.BACKGROUND_COLOR_KEY), UIManager.getColor(BpmnModelerConstants.BACKGROUND_COLOR_KEY).darker());
		}
