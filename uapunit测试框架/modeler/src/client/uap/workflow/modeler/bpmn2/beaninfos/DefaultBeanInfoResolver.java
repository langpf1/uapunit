...
import uap.workflow.modeler.sheet.BeanInfoResolver;
...
public class DefaultBeanInfoResolver implements BeanInfoResolver {
	...
	public BeanInfo getBeanInfo(Object object) {
		...
		if (object instanceof IBeanInfoProvider) {
			...
			try {
				BeanInfo beanInfo = (BeanInfo) Class.forName(beanInfoClassName).newInstance();
				if(beanInfo instanceof GraphBeanInfoWithAddtionalBean){
					((GraphBeanInfoWithAddtionalBean)beanInfo).adjustProperties(object);
				}
				return beanInfo;
			} catch (Exception e) {
