...
import uap.workflow.modeler.sheet.BeanUtilities;
import uap.workflow.modeler.sheet.ExtendedPropertyDescriptor;
 ...
 * Bean的描述信息
 * 
 * <p>修改人：雷军 2004-5-8 修改为使用统一字符串资源管理器 
 * <p>修改人：雷军 2005-3-3 增加方法removeProperty
 */
public class BaseBeanInfo extends SimpleBeanInfo {
  ...
  private List<PropertyDescriptor> properties = new ArrayList<PropertyDescriptor>(0);
  ...
  public PropertyDescriptor removeProperty(String propertyName) {
    ...
    for (Iterator<PropertyDescriptor> iter = properties.iterator(); iter.hasNext();) {
  ...
  public ExtendedPropertyDescriptor addProperty(String propertyName) {
    ...
    try {
      ...
      Method readMethod = BeanUtilities.getReadMethod(getType(), propertyName);
	    ...
	    writeMethod = BeanUtilities.getWriteMethod(
	    	getType(),
	        propertyName,
	        readMethod.getReturnType());
      descriptor = new ExtendedPropertyDescriptor(propertyName, readMethod, writeMethod);

      try {
        // 雷军 修改为使用统一资源管理器 2004-10-08
        //descriptor.setDisplayName(ResManager.getLanguageDependentString(propertyName));
    	  String str = NCLangRes.getInstance().getStrByID("pfworkflow", propertyName);
        ...
        // ignore, the resource may not be provided
			...
			try {
				...
				// + ".shortDescription"));
				String shortDescription = NCLangRes.getInstance().getStrByID("pfworkflow",
