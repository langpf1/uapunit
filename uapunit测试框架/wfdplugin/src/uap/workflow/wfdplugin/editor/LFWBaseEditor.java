...
import uap.workflow.wfdplugin.listener.EventEditorControl;
import uap.workflow.wfdplugin.listener.EventEditorControlComp;
...
//		tabFolder.setSelection(0);
	...
	protected TreeItem getSelectedTreeItem(LFWDirtoryTreeItem dirTreeItem,
		...
		if(editorInput instanceof ElementEditorInput){
			LfwView widget = ((ElementEditorInput)editorInput).getWidget();
			if(widget != null){
				if(widget.getRefId().startsWith("..") || widget.getExtendAttribute(LfwView.POOLWIDGET) != null){//public view
					//public view节点集合
	...
	*/
		...
		if (input instanceof ElementEditorInput) {
			...
			try{
				...
				if(selectedTI == null || selectedTI.isDisposed()){
					...
					if(editorInput.getWidget().getRefId().startsWith("..") || editorInput.getWidget().getExtendAttribute(LfwView.POOLWIDGET) != null){
						widgetTI = getSelectedTreeItem(WEBPersConstants.PUBLIC_VIEW);
					}else{
						widgetTI = getSelectedTreeItem(WEBPersConstants.WINDOW);
					}
	...
	public LFWSeparateTreeItem getWebSeparateTreeItem(String type){
		...
		if(widgetTreeItem != null){
			...
			if(WEBPersConstants.COMPONENTS.equals(type)){
				type = LFWTool.getWEBProjConstantValue("COMPONENTS");
			}else if(WEBPersConstants.DATASET.equals(type)){
				type = LFWTool.getWEBProjConstantValue("DATASET");
			}else if(WEBPersConstants.REFNODE.equals(type)){
				type = LFWTool.getWEBProjConstantValue("REFNODE");
			}else if(WEBPersConstants.COMBODATA.equals(type)){
				type = LFWTool.getWEBProjConstantValue("COMBODATA");
			}
			...
			jump:for (int i = 0; i < separasTreeItems.length; i++) {
				...
				if(item instanceof LFWSeparateTreeItem){
					...
					if(seitem.getText().equals(LFWTool.getWEBProjConstantValue("CONTEXT"))){
